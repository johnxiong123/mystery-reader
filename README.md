...
> 复杂悬疑小说里的人物、关系和时间线越来越多，读到一半总会忘记“这个人到底是谁”。于是我做了这个工具：导入一本小说，AI 自动帮你整理角色、关系网和事件脉络。

总的来说，这个就是一个本地全栈悬疑小说阅读器：导入 txt/epub 后，后端在导入阶段调用 OpenAI 兼容接口抽取人物、关系与事件，阅读阶段只从本地 SQLite 读取并按当前章节服务端防剧透过滤。
...
| 🖥️ **全屏阅读** | 一键切换全屏阅读视图，隐藏右侧面板，专注正文 | ![纯阅读](./screenshots/reader.png) |
...
| 📑 **章节目录** | 抽屉式目录，未读章节标题自动隐藏防剧透，一键跳章 | - |
| 🔎 **防剧透搜索** | Cmd/Ctrl+F 全文搜索，只搜已读范围，绝不泄露后文 | - |
| 🔖 **书签** | 记录章内滚动位置与自动摘录，一键跳回 | - |
| 👆 **选中即查** | 选中正文里的人名，就地弹出人物档案卡 | - |
| 📊 **阅读进度** | 按字数加权显示全书百分比，回看旧章不丢进度 | - |
| 🔒 **防剧透** | 所有数据（含章节标题、搜索片段）服务端按章节过滤，没读到的绝不泄露 | - |
